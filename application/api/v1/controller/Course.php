        ...
        if($this->request->isPost()) {
            ...
            if(!empty($id) && !empty($uid)) {
                ...
                if ($data) {
                    ...
                    $chaFields = 'ch.id,ch.title,date_format(ch.is_time, \'%Y-%m-%d\') as is_time,count(us.id) AS study_num,any_value(IFNULL(st.state,0)) AS state';
                    $chaWhere = [
                        'cp_id' => $id
                    ];
                    ...
                    if ($token = get_wechat_token()) {
                        ...
                        if(copy($url, $path.$file_name)) {
                            ...
                            if(file_exists($path.$file_name)) {
                                ...
                                $result = upload_file($file_name, $path.$file_name);
