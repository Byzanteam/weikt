...
function get_user_rank_no ($type=1, $id=0, $time=0, $limit=10) {
    ...
    $sql = 'SELECT basic.name, basic.id as user_id, IFNULL(num,0) as num, IFNULL(rank_no,10) as rank_no FROM vcr_user_basic basic LEFT JOIN (' . $rankSql . ') c_tmp ON basic.id=c_tmp.user_id' . $where . ' ORDER BY num DESC ' . $limit;
